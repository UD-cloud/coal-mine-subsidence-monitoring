...
import { LayoutDashboard, Map, BrainCircuit, BellRing, RadioTower, Database, Settings, Activity, ChevronRight } from "lucide-react";
import Logo from "./Logo";

const items = [
  ["Dashboard","/",LayoutDashboard],["Live Map","/map",Map],["AI Intelligence","/ai",BrainCircuit],["Alerts","/alerts",BellRing],["Sensor Network","/sensors",RadioTower],["Data & Reports","/reports",Database]
];
  ...
  return <aside className="fixed inset-y-0 left-0 z-40 hidden w-[252px] border-r border-white/5 bg-[#07110f]/95 px-4 py-5 lg:block">
    ...
    <nav className="mt-3 space-y-1">
      {items.map(([label,path,Icon])=><NavLink key={path} to={path} className={({isActive})=>`group flex items-center gap-3 rounded-xl px-3 py-3 text-sm transition ${isActive?"bg-emerald-300/10 text-emerald-200":"text-slate-400 hover:bg-white/5 hover:text-white"}`}>
        <Icon className="h-[18px] w-[18px]"/><span className="flex-1">{label}</span>{label==="Alerts"&&<span className="grid h-5 min-w-5 place-items-center rounded-full bg-red-400/15 px-1 text-[10px] text-red-300">5</span>}<ChevronRight className="h-3 w-3 opacity-0 transition group-hover:opacity-50"/>
      </NavLink>)}
    </nav>
