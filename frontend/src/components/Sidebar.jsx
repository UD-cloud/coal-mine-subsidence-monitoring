...
export default function Sidebar() {
  ...
  return (
    <aside
      className="
        fixed inset-y-0 left-0 z-40 hidden w-[252px]
        flex-col
    ...
    >
      ...
      {/* Operations */}
      <div className="mt-10 px-3 text-[10px] font-bold uppercase tracking-[.25em] text-slate-600">
        Operations
      </div>
      ...
      <nav className="mt-3 space-y-1">
        {items.map(([label, path, Icon]) => (
          ...
          >
            ...
            {/* Offline nodes count */}
            {label === "Dead Nodes" && offlineCount > 0 && (
              <span className="grid h-5 min-w-5 shrink-0 place-items-center rounded-full bg-red-400/15 px-1 text-[10px] text-red-300">
                {offlineCount}
              </span>
            )}

            <ChevronRight className="h-3 w-3 shrink-0 opacity-0 transition group-hover:opacity-50" />
          </NavLink>
        ))}
      </nav>

      {/* System */}
      <div className="mt-8 px-3 text-[10px] font-bold uppercase tracking-[.25em] text-slate-600">
        System
      </div>

      <NavLink
        to="/settings"
        className="mt-3 flex items-center gap-3 rounded-xl px-3 py-3 text-sm text-slate-400 hover:bg-white/5 hover:text-white"
      >
        <Settings className="h-[18px] w-[18px] shrink-0" />
        <span>Settings</span>
      </NavLink>

      {/* Push status card toward bottom */}
      <div className="mt-auto pt-6">
        <div className="rounded-2xl border border-emerald-300/10 bg-emerald-300/[.04] p-4">
          <div className="flex items-center gap-2 text-xs font-semibold text-emerald-200">
            <span className="h-2 w-2 shrink-0 rounded-full bg-emerald-300 shadow-[0_0_12px_#8ff0c0]" />
            <span>Network operational</span>
          </div>

          <div className="mt-2 text-[11px] text-slate-500">
            Last sync · 18:01:42 IST
          </div>
        </div>
      </div>
    </aside>
